...
const SignUp: React.FC<SignUpProps> = ({ onComplete }) => {
  ...
  const toggleInterest = (interest: InterestTopic, slot: number) => {
    ...
    if (!selected && form.interests.length >= MAX_INTERESTS) {
      return;
    }
  ...
  const validateStep = () => {
    ...
    if (step === 3 && form.interests.length < MIN_INTERESTS) {
  ...
  const currentInterestSubgroup = currentInterestGroup && interestPath.subgroupId
    ...
    : undefined;
  const interestLimitReached = form.interests.length === MAX_INTERESTS;
  ...
  const interestNavigationItems = useMemo<InterestNavigationItem[]>(() => {
    ...
    return currentInterestSubgroup.topics
      .filter((interestTopic) => !form.interests.includes(interestTopic.id))
      .map((interestTopic) => ({
  ...
  const interestCanvas = useMemo<Array<InterestCanvasItem | null>>(() => {
    const selectedTopics = form.interests
    ...
    return canvas;
  }, [form.interests, interestNavigationItems, interestSlots]);
  ...
  const canContinue = step !== STEPS.length - 1 || form.interests.length >= MIN_INTERESTS;
          ...
          <form className="signup-card" onSubmit={handleSubmit}>
            ...
            <div className="signup-actions">
              {isInsideInterestNavigation ? (
                <IonButton
                  key={`back:${interestPath.groupId}:${interestPath.subgroupId ?? ''}:${form.interests.length}`}
                ...
                <IonButton
                  key={`continue:${form.interests.length}:${canContinue}`}
